...
- [x] check NUC with USB2 cable before taking up to Observatory
	- [x] current position Nov '25 after tests is still not good - long download time. Points to NUC fault. Also the previous cables in use WERE USB2.
...
- [x] Battery updates - see [[Lifepo4 batteries]] 
- [x] update the Pyxis 2 universal driver [here](https://www.optecinc.com/downloads/rotators/universal/) in the lenovo & Nuc machine
- [x] find psu for Sitech controller
- [x] get vaseline for battery terminals
- [x] pick optec pyxis Universal ASCOM for the connection in PHD2 and in NINA and see if the connection will share
...
- [x] At the Pyxis, we have a tube (at home) with a dovetail at one end and an M54 thread at the other. It looks like FLO have an adapter that will adapt this to M48 male which will then fit the M48 female end of the new spacer stack, had one built at cross engineering
- [x] Install new cam and test focus
- [x] find shims?
...
- [x] ~={red}On the night of 17th May, guiding was good then bad with no star seemingly available and focussing half flux radius in SGP was all over the place, with masses of 'stars' so do the following=~
...
- [x] Check the meridian flip settings in Sitech don't conflict with SGP - that may have been the problem on 5-5-25
- [ ] 
